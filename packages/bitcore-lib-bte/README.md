# Bitwebcore JavaScript Library for Bitweb
...
Bitweb is a powerful new peer-to-peer platform for the next generation of financial technology. The decentralized nature of the Bitweb network allows for highly resilient bitweb infrastructure, and the developer community needs reliable, open-source tools to implement bitweb apps and services. Bitwebcore JavaScript Library provides a reliable API for JavaScript apps that need to interface with Bitweb.

## Get Started

Clone the Bitcore monorepo and `npm install`:
```sh
git clone https://github.com/bitweb-project/bitcore.git
npm install
```
`cd` into bitwebcore-lib repository:
```sh
cd packages/bitwebcore-lib
```

## Building the Browser Bundle

To build a bitwebcore-lib full bundle for the browser:

```sh
gulp browser
```

This will generate files named `bitwebcore-lib.js` and `bitwebcore-lib.min.js`.
...
We're using the Bitwebcore JavaScript Library in production, as are many others, but please use common sense when doing anything related to finances! We take no responsibility for your implementation decisions.
...
Copyright 2021-2023 Bitweb Project, Bitwebcore is bitcorelib implementation for Bitweb.