...
const { BitcoreLibBte } = require('crypto-wallet-core');
const { ChainService } = require('../../ts_build/lib/chain');
const { BteChain } = require('../../ts_build/lib/chain/bte');
...
describe('Chain BTE', () => {
  ...
  describe('#getEstimatedSize', () => {
    let bte, fromAddress, simpleUtxoWith1BTE, changeAddress, toAddress, privateKey;
    ...
    before(() =>  {
      ...
      simpleUtxoWith1BTE = {
        ...
        outputIndex: 0,
        script: BitcoreLibBte.Script.buildPublicKeyHashOut(fromAddress).toString(),
        satoshis: 1e8,
      };

      const privKey = new BitcoreLibBte.PrivateKey();
    });
    ...
    it('1 input p2pkh,1 output p2pkh: Margin should be 10%', () => {
      ...
      const estimatedLength = bte.getEstimatedSize(x);

      // Create a similar TX.
      let tx = new BitcoreLibBte.Transaction();
      tx.from(simpleUtxoWith1BTE)
    ...
    const p2shPrivateKey1 = BitcoreLibBte.PrivateKey.fromWIF('T4EAFWF8i3vFtgXW8nwRQWgSo2E3VEp5D3vbv27umAjUCQQrsqFQ');
    const p2shPublicKey1 = p2shPrivateKey1.toPublicKey();
    const p2shPrivateKey2 = BitcoreLibBte.PrivateKey.fromWIF('T4PcE9qqC9UNhB5Xb694epiXXEYW6xp6aA8QHT4UB83UfcDVicew');
    const p2shPublicKey2 = p2shPrivateKey2.toPublicKey();

    const p2shAddress = BitcoreLibBte.Address.createMultisig([
      p2shPublicKey1,
      p2shPublicKey2,
    ], 2, 'testnet');
    const p2shUtxoWith1BTC = {
      ...
      txId: 'a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458',
      outputIndex: 0,
      script: BitcoreLibBte.Script(p2shAddress).toString(),
      satoshis: 1e8
    };
